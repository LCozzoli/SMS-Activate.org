...
const query_module_1 = require("./query.module");
let Countries = class Countries {
    constructor(query) {
        ...
        if (!this.list ||
            !this.list.updatedAt ||
            this.list.updatedAt < Date.now() - 1000 * 60 * 60 * 24 * 30)
            this.update(COUNTRY_FILE);
    }
    update(file) {
        this.query
            .makeCall(comon_1.EApiActions.getCountries, null)
            .then((countries) => {
            if (countries && countries.length) {
                ...
                (0, fs_1.writeFileSync)(file, JSON.stringify(countries));
            }
        });
    }
    get(name, lang) {
